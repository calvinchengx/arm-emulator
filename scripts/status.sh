...
# answers that. `docker compose ps` knows containers but not whether ARM
# actually serves; and a container can be up AND healthy while attached to NO network,
...
check_http "$ARM/health" "arm /health" 200

# The one check that proves the PAIR is wired, not just that both processes are
# alive: a tokenless ARM call must be refused with a 401 whose
# WWW-Authenticate names entra's authority. That challenge is what every Azure
# client follows to acquire a token, so if it is missing or points somewhere
# else, `az login` and DefaultAzureCredential fail no matter how healthy both
# containers look.
say ""
say "challenge handshake (what the Azure clients actually follow)"
hdr=$(curl -skI --max-time 5 "$ARM/subscriptions?api-version=2022-04-01" 2>/dev/null || printf '')
...
# Cloud discovery is what `az cloud register` fetches first, anonymously. If
# this is wrong the CLI cannot even register the emulator family as a cloud.
say ""
say "cloud discovery (what az cloud register fetches)"
meta=$(curl -sk --max-time 5 "$ARM/metadata/endpoints" 2>/dev/null || printf '')
case "$meta" in
  *loginEndpoint*) printf '  ok    %-24s advertises a login endpoint\n' "/metadata/endpoints" ;;
  *)               printf '  FAIL  %-24s missing or malformed\n' "/metadata/endpoints"; bad ;;
esac
