...
WHY THIS EXISTS. The docs moved from `/arm-emulator/<slug>/` to
`/arm-emulator/docs/<slug>/`, which is the shape the rest of the
family uses. That moved **20 published routes**, and a moved URL is a 404 for
every link already pointing at it: this project's own README, anything in the
sibling repositories, and anything outside them nobody can enumerate. All 20
are 404 on the live site today, measured. Grepping the family found no
remaining links to them, which says nothing about the ones outside it: those
...
`/arm-emulator/docs/01-quickstart/` and nothing answers the
...
# project's 20 published routes keeps its own name under /docs/. The one route
