...
check() {
  ...
  got=$(
    ...
    GITHUB_REPOSITORY="calvinchengx/arm-emulator" \
...
README.md
'
check false "the docs site" pull_request \
...
# --- the invariant the docs set depends on ----------------------------------
# `docs/` is inert only because the job that READS it is not gated. If that
# ever grows an `if: needs.changes...`, the edit that broke a parity claim
# would be exactly the edit that skipped the check on it.
wf="$HERE/workflows/ci.yml"
if grep -qE "^  witnesses:" "$wf"; then
  body=$(awk '/^  witnesses:/{f=1;next} /^  [a-z][a-z0-9_-]*:/{f=0} f' "$wf")
  if printf '%s' "$body" | grep -q "needs.changes.outputs.code"; then
    fail=$((fail+1))
    printf '  FAIL  %-52s it reads the docs it would skip\n' "witnesses stays ungated"
  else
    pass=$((pass+1)); printf '  ok    %-52s ungated\n' "witnesses stays ungated"
  fi
else
  fail=$((fail+1)); printf '  FAIL  %-52s job not found in %s\n' "witnesses stays ungated" "$wf"
fi
