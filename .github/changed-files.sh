...
#
# WHAT KEEPS THE DOCS SET SAFE HERE. `docs/` is inert for the GATED jobs and
# only for those: `witnesses` in ci.yml reads it (check_witnesses.py reads docs/parity.md and docs/witnesses.json) and carries no
# `if:`, so it runs on every change, docs or code. Gating it would skip the
# only check that reads the file being edited. changed-files.test.sh asserts
# it stays ungated.
#
