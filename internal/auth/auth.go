// Package auth validates Entra bearer tokens the way real ARM does:
// signature against the issuer's JWKS, issuer match, the ARM audience, and
...
// signature verifies. Shared shape with the sibling emulators.
package auth
...
// ARMAudiences are the audiences the management plane accepts (Azure keeps
// the trailing-slash variant when a client requests it).
var ARMAudiences = []string{
...
type Principal struct {
	...
	Groups []string // groups claim (entra-emulator >= v0.3.0); may be empty
}
...
func NewMulti(pairs [][2]string, insecure bool, now func() int64, client *http.Client) *Validator {
	...
	v := &Validator{Audiences: ARMAudiences, Now: now, client: client}
	for _, p := range pairs {
