...
func TestValidateHappyPathUserAndApp(t *testing.T) {
	...
	base := mintOpts{iss: testIssuer, aud: ARMAudiences[0], exp: 2000, kid: "test-key"}
	...
	slash := base
	slash.aud = []string{ARMAudiences[1]}
	slash.oid = "oid-2"
...
func TestValidateRejections(t *testing.T) {
	v, key := newFixture(t, 1000)
	good := mintOpts{iss: testIssuer, aud: ARMAudiences[0], exp: 2000, oid: "o", kid: "test-key"}
...
func TestExpiryFollowsEmulatorClock(t *testing.T) {
	...
	tok := mint(t, key, mintOpts{iss: testIssuer, aud: ARMAudiences[0], exp: 2000, oid: "o", kid: "k"})
...
func TestValidateRequest(t *testing.T) {
	...
	tok := mint(t, key, mintOpts{iss: testIssuer, aud: ARMAudiences[0], exp: 2000, oid: "o", kid: "test-key"})
