...
// The unset/set-empty distinction IS the feature: leaving ARM_DATA_DIR alone
...
func TestDataDirDefaulting(t *testing.T) {
	t.Setenv("ARM_ENTRA_ISSUER", "https://localhost:8443/t/v2.0")

	t.Run("unset persists to the default dir", func(t *testing.T) {
		t.Setenv("ARM_DATA_DIR", "") // ensure t.Setenv restores whatever was there
		if err := os.Unsetenv("ARM_DATA_DIR"); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("a path is honoured", func(t *testing.T) {
		t.Setenv("ARM_DATA_DIR", "/tmp/arm-state")
		if got := FromEnvPartial().DataDir; got != "/tmp/arm-state" {
